...
(() => {
  ...
  const STORAGE_NS = 'aimated-cards-playground';

  const PROJECT_NAME =
    (document.body && document.body.dataset.project) ||
    // CHANGED: query param key now 'cards'
    new URLSearchParams(location.search).get('animated-cards') ||
...
/* Tip: static imports must remain top-level (no try/catch wrappers) */
  ...
  (function boot() {
    ...
    if (saved) {
      ...
      // PREFILL with Evervault-style scanner demo (only on first load with no saved state)
...
<div class="controls">
  ...
  <button class="control-btn" onclick="changeDirection()">↔️ Direction</button>
</div>
...
<!-- three.js CDN (needed for ParticleSystem) -->
